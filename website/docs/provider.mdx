...
example, if `--react` was passed, we can enable the React preset. _However_, this is only available
when your configuration strategy is create, as we own the process, and _will not_ be available for
copy/reference strategies.
